    ...
    <div style={{ textAlign: 'center' }}>
      {isAuthenticated ? (
        ...
        <div>
          ...
          <input type="password" placeholder="pwd" value={password} onChange={(e) => setPassword(e.target.value)} />
          <button onClick={handleLogin}>Log In</button>
          <p>Log in for detailed report.</p>
        </div>
      )}

      {pdfImage && (
        <PDFViewer width="100%" height={600}>
          <Pdf pdfImage={pdfImage} pdfStats={pdfStats} />
        </PDFViewer>
      )}
    </div>
  );
